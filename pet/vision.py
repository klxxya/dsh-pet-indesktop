...
# 默认读屏补充指令（设置里可覆盖）：含自我认知（画面里的桌宠是自己）
DEFAULT_EXTRA_PROMPT = (
    '补充：画面里如果有一只 Q 版蓝发鲸鱼女仆小桌宠，那就是你自己（本机桌宠本体），'
    '可以自然地认出自己并自嘲或自夸，别把自己当成别人家的图片角色。'
)
...
def ask_about_screen(image_path, app_info: str, system_prompt: str, p: ProviderConfig,
    ...
    user_text = (
        ...
        + (extra_prompt.strip() or DEFAULT_EXTRA_PROMPT)
    )
    payload = {
