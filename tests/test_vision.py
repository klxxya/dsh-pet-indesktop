...
def test_extra_prompt_override_and_default():
    """用户补充指令覆盖默认；空值回退默认（含认出自己）。"""
    from pet.vision import DEFAULT_EXTRA_PROMPT
    import inspect
    from pet import vision
    src = inspect.getsource(vision.ask_about_screen)
    assert 'extra_prompt' in src and 'DEFAULT_EXTRA_PROMPT' in src
    assert '认出' in DEFAULT_EXTRA_PROMPT
